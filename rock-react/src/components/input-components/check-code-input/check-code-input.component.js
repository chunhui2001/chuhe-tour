...
class CheckCodeInputComponent extends Component {
    ...
    render() {
        ...
        const { placeholder, phoneOrEmail } = this.props;
        ...
        return (
            <div className="check-code-input-component input-group">
                ...
                <input className={"input-group-field " + (this.state.checkcodeInvalid ? "checkcode-invalid": "")} 
                        type="text" pattern="[a-z0-9]{1,15}" 
                        ref={input => this.checkCodeInput = input}
                        onChange={this.onCheckCodeChange} 
                        onKeyDown={this.onCheckCheckCode}
                        placeholder={placeholder} id="check_code" name="check_code" required />

                <input type="hidden" id="check_code_sign" name="check_code_sign" />
                <input type="hidden" id="validate_code_sign" name="validate_code_sign" />

                <span className="input-group-label"
                        onClick={() => this.changeSteps('clicked_send_click')}
                        style={{
                            cursor: 'pointer', padding: '0 .425rem', color: 'blueviolet', 
                            borderLeft: 'none', backgroundColor: 'antiquewhite',
                            display: this.state.defaultStep === 'pre_send_click'? 'flex' : 'none'}}>
                        发送验证码
                </span>
                <span className="input-group-label checkcode " 
                      onClick={this.changeCheckCodeImage}
                      style={{ width: '104px', borderLeft: 'none', backgroundSize: '100% 100%',
                               display: this.state.defaultStep === 'clicked_send_click'? 'flex' : 'none',
                               backgroundImage: 'url("' + '/checkcode?sign=' + this.state.checkCodeSign + '&receiver=18500183080' + '")'}}>
                </span>
                <span className="input-group-label checkcode-valid-button checkcode-correct" 
                      style={{display: 'none'}}>
                        &ensp;验证&ensp;
                </span>
                <span className="input-group-label checkcode-time-button"
                        style={{width: 'auto', borderLeft: 'none',display: 'none'}}>
                        重试
                </span>
                <span className="input-group-label checkcode-time-button" style={{display: 'none'}}>
